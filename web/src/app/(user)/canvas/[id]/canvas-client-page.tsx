        ...
        async (nodeId: string, mode: CanvasNodeGenerationMode, prompt: string) => {
            ...
            try {
                if (mode === "image") {
                    ...
                    let firstFailureErrorDetails: string | undefined;
                    await Promise.all(
                        targetIds.map(async (targetId) => {
                            try {
                                ...
                                const errorDetails = error instanceof Error ? error.message : "生成失败";
                                if (!firstFailureErrorDetails) firstFailureErrorDetails = errorDetails;
                                hasFailure = true;
                    ...
                    const aggregateErrorDetails = firstFailureErrorDetails || "全部图片生成失败";
                    setNodes((prev) =>
                        prev.map((node) =>
                            node.id === nodeId && isConfigNode
                                ? { ...node, metadata: { ...node.metadata, status: hasSuccess ? NODE_STATUS_SUCCESS : NODE_STATUS_ERROR, errorDetails: hasSuccess ? undefined : aggregateErrorDetails } }
                                : node.id === nodeId && isEmptyImageNode
                                  ? { ...node, metadata: { ...node.metadata, status: hasSuccess ? NODE_STATUS_SUCCESS : NODE_STATUS_ERROR, errorDetails: hasSuccess ? undefined : aggregateErrorDetails } }
                                  : node.id === rootId && !hasSuccess
                                    ? { ...node, metadata: { ...node.metadata, status: NODE_STATUS_ERROR, errorDetails: aggregateErrorDetails } }
                                    : node,
                        ),
                    );
                    return;
